  ...
  <div v-if="column.getCanSort()" :class="cn('flex items-center space-x-2', $attrs.class ?? '')">
    <DropdownMenu>
      <DropdownMenuTrigger as-child>
        <Button
            :class="cn('-ml-3 h-8 data-[state=open]:bg-accent', column.getIsSorted() && 'text-primary bg-gray-100')"
            size="sm"
            variant="ghost"
        >
          <span>{{ title }}</span>
      ...
      <DropdownMenuContent align="start">
        ...
        <DropdownMenuItem @click="column.clearSorting()">
          <ArrowUpDownIcon class="mr-2 h-3.5 w-3.5 text-muted-foreground/70"/>
          None
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
